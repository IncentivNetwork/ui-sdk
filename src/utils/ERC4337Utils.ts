...
const UserOpType = EntryPoint__factory.abi.find(entry => entry.name === validateUserOpMethod)?.inputs[0]
if (UserOpType == null) {
  throw new Error(`unable to find method ${validateUserOpMethod} in EP ${EntryPoint__factory.abi.filter(x => x.type === 'function').map(x => x.name).join(',')}`)
}
