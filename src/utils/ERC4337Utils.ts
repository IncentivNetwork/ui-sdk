...
const PackedUserOpType = IEntryPoint__factory.abi.find(entry => entry.name === getUserOpHashMethod)?.inputs[0]
if (PackedUserOpType == null) {
  throw new Error(`unable to find method ${getUserOpHashMethod} in EP ${IEntryPoint__factory.abi.filter(x => x.type === 'function').map(x => x.name).join(',')}`)
}
